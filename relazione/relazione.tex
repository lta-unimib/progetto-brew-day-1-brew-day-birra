...
Sono state svolte le seguenti attività:
	\begin{itemize}
		...
		\item Daily Scrum, meeting online giornaliero di 10 minuti in cui ci siamo confrontati sul lavoro svolto il giorno precedente e pianificato i compiti del giorno per il raggiungimento dello Sprint Goal
...
\newpage
	...
	All'inizio del progetto sono stati individuati i requisiti funzionali e a ciascuno è stata attribuita un'importanza, come prevede la tecnica MoSCoW (tabella \ref{ab:Requisiti-Funzionali}).
    		\begin{table}[!h]
      			\renewcommand{\arraystretch}{1.2}
	...
	Oltre ai requisiti funzionali, abbiamo individuato anche quelli non funzionali (tabella \ref{ab:Requisiti-Non-Funzionali}). 
    		\begin{table}[!h]
      			\renewcommand{\arraystretch}{1.2}
      			\begin{tabular}{p{0.8\textwidth}|p{0.1\textwidth}} 
        				\textbf{Requisito}  & \textbf{MoSCoW} \\
    				\hline
                                  il sistema deve mantenere le quantita' degli ingredienti nelle ricette (e nell'inventario) in termini di unita' assolute (anche diverse), in modo che sia più semplice calcolare i multipli & M \\
	...
	Il modello di dominio realizzato e riportato nella figura \ref{fig:Modello-Dominio} fornisce una rappresentazione visuale delle classi concettuali e degli oggetti del mondo reale del dominio.
		\begin{figure}[!h]
			\centering
	...
	I casi scelti sono l'aggiunta di una ricetta nel sistema e la sua modifica.
      ...
      \chapter{Progettazione}
		...
		\subsection{Diagramma delle Classi Generation}
			\begin{figure}[!h]
				\centering
				\includegraphics[width=01\linewidth]{image/Diagramma-delle-Classi-Generation.png}
    ...
    \newpage
		...
		\subsection{SD - modificaQuantitaIngrediente}
			E' stato realizzato il diagramma di sequenza di modificaQuantitaIngrediente appartenente al caso d'suo modificaRicetta
			\begin{figure}[!h]
				\centering
		...
		\subsection{SD - modificaNomeIngrediente}
			E' stato realizzato il diagramma di sequenza di modificaNomeIngrediente appartenente al caso d'suo modificaRicetta
			\begin{figure}[!h]
				\centering
		...
		\begin{figure}[!h]
			...
			\caption{Diagramma degli Stati Birra}\label{fig:1}
		\end{figure}    			
		...
		\begin{figure}[!h]
			...
			\caption{Diagramma delle Attivita di Esecuzione Ricetta}\label{fig:1}
		\end{figure}    
\newpage	
...
Implementano questo pattern anche le classi Validator, ValidatorHandler e le classi IDGenerator e derivate. 
	...
	Abbiamo usato SonarQube e SonarCloud per mantenere controllato il livello di qualità del progetto.\\
		\begin{figure}[!h]
			\centering
		...
		\end{figure} 
	Dalle analisi finali svolte, i cui risultati sono riportati nelle figure \ref{fig:sonarCloud-backend} \ref{fig:sonarCloud-frontend}, si può affermare che il progetto non presenta bugs, codeSmells, vulnerabilità e problemi di sicurezza.
...
\newpage
	...
	Abbiamo utilizzato il tool Understand per capire se nel progetto fossero presenti anti-pattern strutturali legati a problemi di dipendenza o altri problemi.
		\begin{figure}[!h]
			...
			\caption{Understand}\label{fig:1}
		\end{figure}    	
